...
namespace ScheduleOrder.Utils
{
   /// <summary>
    ...
    public class PrintHelper
    {
        ...
        public void Print(DataTable dt, string title, DateTime startWeek)
        {
            try
            {
                CreatePrintDocument(dt, title,startWeek).Print();
            }
            catch (Exception ex)
            {
        ...
        /// <param name="dt">要打印的DataTable</param>
        /// <param name="title">打印文件的标题</param>
        public void PrintPriview(DataTable dt, string title, DateTime startWeek)
        {
            try
            {
                var printPriview = new PrintPreviewDialog
                    {
                        Document = CreatePrintDocument(dt, title, startWeek),
        ...
        //private PrintDocument CreatePrintDocForReview() { }
        /// <summary>
        ///     创建打印文件
        /// </summary>
        private PrintDocument CreatePrintDocument(DataTable dt, string title, DateTime startWeek)
        {
            _dataTable = dt;
            _headText = title;
            ...
            _printDocument.DefaultPageSettings.PaperSize = new PaperSize("A4", 595, 842);
            ...
            if (_printingPageNumber == 1)
            {
                ...
                if (arr.Length > 1)
                {
                    e.Graphics.DrawString(arr[0],
                        _headFont,
                        _drawBrush,
                        new Point(e.PageBounds.Width / 2, _pageTop), font);
                }
                //副标题
                ...
                for (int i = 1; i < arr.Length; i++)
                {
                    e.Graphics.DrawString(arr[i],
                        ...
                        _drawBrush,
                        new Point(e.PageBounds.Width / 2, _pageTop + _headFont.Height),
                        font);
